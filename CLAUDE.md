...
Arrow Escape — 단일 파일(`index.html`) HTML5 캔버스 퍼즐 게임. 빌드·의존성 없음.
격자에 얽힌 뱀 모양 화살표를 클릭하면 머리 방향 직선으로 탈출하고, 막히면 튕겨
돌아온다. 모든 화살표를 내보내면 클리어. UI·주석·문서·사용자 대화 모두 한국어.

## 실행과 검증

- 실행: `index.html`을 브라우저에서 열면 끝.
- 자동화 검증(Playwright MCP): `file://`이 차단되므로 `python3 -m http.server 8931`
  (백그라운드)로 서빙한 뒤 `http://localhost:8931/index.html` 접속.
