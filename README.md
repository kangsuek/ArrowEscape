...
- **타임어택 모드**(스톱워치 아이콘으로 켜기/끄기): 시작 시 "TIME ATTACK START!"
  배너가 중앙에서 확대되며 사라진다. 화살표가 탈출할 때마다 +1초, 레벨 클리어
  시 보너스 시간이 추가로 붙어 레벨을 이어 달린다. 오클릭은 시간을 깎는다.
  남은 시간이 10초 이하가 되면 초 단위로 숫자가 확대되는 펄스 연출과 120bpm 고정
  템포의 긴장감 BGM이 재생되고, 타임오버 시 엔딩 징글이 나온다.
- **온라인 순위표**(타임오버 화면 전용): 이름·자기선택 국가(국기 표시)를 입력해
  점수를 등록하면 등수와 상위 10위 목록을 볼 수 있다(Firebase Realtime Database,
  REST API 로만 연동해 별도 SDK·빌드 없음). 위치정보를 수집하지 않으며, 이름은
  간단한 금칙어 필터를 거친다.
